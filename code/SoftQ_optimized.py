...
class SofQData:
    ...
    def __init__(self, udp_host_port_to, udp_host_port_back):
        """
        Initializes the SensorController class and configures the ToF sensors.
        """
        self.udp_host_port = (udp_host_port_to, udp_host_port_back)
...
class SoftQ:
    ...
    def __init__(self, useJoy=False):
        ...
        if useJoy:
            self.data = SofQData((HOST4, PORT4), (HOST3, PORT3))
            self.initialize_joystick()
        else:
            self.data = SofQData((HOST2, PORT2), (HOST1, PORT1))
    ...
    def start(self):
        ...
        timer_write_simulink = perpetualTimer(0.051, self.UDP_Transmit, 'UDP_Transmit')
    ...
    def UDP_Transmit(self):
        ...
        if self.use_joystick == True:
            ...
            data_to_Simulink = self.sensor_data.get_data_to_simulink(True, Axis_Readings, Button_Readings)
        else:
            data_to_Simulink = self.sensor_data.get_data_to_simulink(False)
#         print(list(data_to_Simulink))
        try:
            self.sock2.sendto(data_to_Simulink, self.data.udp_host_port[0])
        except:
...
#             print("data_to_STM32",list(data_to_stm32))
    ...
    def serial_setup(self):
        ...
        self.sock1.bind(self.data.udp_host_port[1])
